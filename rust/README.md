...
`pi-zig-sys` compiles `zig-kernel` from `build.rs`, then Rust links the generated static library. `pi-zig-codegen` also runs a host Zig program that uses `comptime` descriptors to generate Rust source under Cargo `OUT_DIR`; the Rust macro shell exposes that generated table.
