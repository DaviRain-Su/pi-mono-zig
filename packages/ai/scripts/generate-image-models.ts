...
function generateImageModelsFile(models: ImagesModel<"openrouter-images">[]): string {
	const formatStringArray = (values: readonly string[]): string => `[${values.map((value) => JSON.stringify(value)).join(", ")}]`;
	const formatPropertyKey = (key: string): string => (/^[$A-Z_a-z][$\w]*$/.test(key) ? key : JSON.stringify(key));
	const formatCost = (cost: ImagesModel<"openrouter-images">["cost"]): string => `{
				input: ${cost.input},
				output: ${cost.output},
	...
	const providerEntries = Object.entries(imageModelsByProvider)
		.map(([provider, providerModels]) => {
			const modelEntries = Object.entries(providerModels)
				...
				.join("\n");
			return `\t${formatPropertyKey(provider)}: {\n${modelEntries}\n\t},`;
		})
		.join("\n");
