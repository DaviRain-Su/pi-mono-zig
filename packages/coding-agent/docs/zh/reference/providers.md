...
> 认证细节补充：
> - `anthropic` 在解析环境变量时，`ANTHROPIC_OAUTH_TOKEN` 优先于 `ANTHROPIC_API_KEY`
> - `github-copilot` 还可使用 `COPILOT_GITHUB_TOKEN`、`GH_TOKEN` 或 `GITHUB_TOKEN`
> - `google-vertex` 不仅需要 ADC 凭据，还需要 `GOOGLE_CLOUD_PROJECT`（或 `GCLOUD_PROJECT`）与 `GOOGLE_CLOUD_LOCATION`
