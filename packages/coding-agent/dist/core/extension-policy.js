...
import { WASM_CANONICAL_SECURITY_GRANTS, } from "./wasm-extension-package.js";
...
const GRANTS = new Set(CANONICAL_EXTENSION_GRANTS);
...
function createLegacyWasmExtensionIdentityKey(manifest) {
    return `wasm:${manifest.schemaVersion}:${manifest.id}:${manifest.version}:${manifest.artifactSha256}:${toPolicyPath(manifest.manifestPath)}:${toPolicyPath(manifest.artifactAbsolutePath)}`;
}
...
export function createWasmExtensionLegacyPolicyKey(manifest) {
...
function relativePolicyPath(baseDir, filePath) {
    if (!baseDir)
        return undefined;
...
export function isCanonicalExtensionGrant(value) {
    return GRANTS.has(value);
}
export function hasExtensionGrant(policy, grant) {
    return policy?.approvedGrants?.includes(grant) === true;
}
export function createExtensionPolicyDenialError(identity, capability, operation, target) {
...
export function assertExtensionGrant(identity, policy, capability, operation, target) {
    if (hasExtensionGrant(policy, capability))
        return;
    throw createExtensionPolicyDenialError(identity, capability, operation, target);
}
function formatExtensionPolicyDenialMessage(details) {
...
export function createTypeScriptExtensionIdentity(options) {
    ...
    if (sourceInfo.origin === "package") {
        ...
        const key = provenance
            ? `typescript:package:${sourceInfo.scope}:${provenance.sourceIdentity}:${entryPath}:${toPolicyPath(provenance.packageRoot)}:${provenance.packageRootSha256}:${resolvedPath}`
...
export function createWasmExtensionIdentity(manifest, sourceInfo) {
    ...
    const key = provenance
        ...
        })}${sourceInfo.scope}:${provenance.sourceIdentity}:${toPolicyPath(provenance.packageRoot)}:${provenance.packageRootSha256}:${artifactSha256}:${manifestPath}:${artifactAbsolutePath}`
...
export function validateExtensionPolicyShape(value, path = "$") {
...
export function validateExtensionPolicyMap(value, path = "$") {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        throw new Error(`${path}: expected object`);
    }
    const policies = {};
    const errors = [];
    for (const [identity, policyValue] of Object.entries(value)) {
...
export function mergeExtensionPolicy(base, override) {
    const merged = {};
    ...
    if (override.resourceLimits !== undefined) {
        merged.resourceLimits = {
            ...
            toolScopes: override.resourceLimits.toolScopes !== undefined
...
export function mergeExtensionPolicyMaps(base, overrides) {
    if (!base && !overrides) {
        return undefined;
    }
    const merged = {};
...
function validateResourceLimits(value, path) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        throw new Error(`${path}: expected object`);
    }
    const object = value;
...
function setResourceLimitInteger(limits, object, path, field) {
    const value = optionalResourceLimitInteger(object, path, field);
    if (value !== undefined) {
        limits[field] = value;
    }
}
function optionalResourceLimitInteger(object, path, field) {
...
function optionalToolScopes(object, path) {
...
export function normalizeWasmResourceLimits(limits) {
    return {
