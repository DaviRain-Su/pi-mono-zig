...
import { mergeExtensionPolicyMaps, validateExtensionPolicyMap, validateExtensionPolicyShape, } from "./extension-policy.js";
...
export class SettingsManager {
    ...
    static fromStorage(storage) {
        ...
        return new SettingsManager(storage, globalLoad.settings, projectLoad.settings, globalLoad.errors[0] ?? null, projectLoad.errors[0] ?? null, initialErrors);
    }
    ...
    static migrateSettings(settings, errors = []) {
    ...
    static validateExtensionPoliciesSetting(settings, errors) {
    ...
    static parseCurrentSettingsForWrite(current) {
        const errors = [];
        const settings = SettingsManager.migrateSettings(JSON.parse(current), errors);
        if (errors.length > 0) {
            throw errors[0];
        }
        return settings;
    ...
    getExtensionPolicy(identityKey) {
        const policy = this.settings.extensionPolicies?.[identityKey];
        return policy ? structuredClone(policy) : undefined;
    }
    setExtensionPolicy(identityKey, policy) {
        this.validateExtensionPolicyIdentity(identityKey);
        const normalizedPolicy = validateExtensionPolicyShape(policy, `$.extensionPolicies[${JSON.stringify(identityKey)}]`);
    ...
    setProjectExtensionPolicy(identityKey, policy) {
        this.validateExtensionPolicyIdentity(identityKey);
        const normalizedPolicy = validateExtensionPolicyShape(policy, `$.extensionPolicies[${JSON.stringify(identityKey)}]`);
        const projectSettings = structuredClone(this.projectSettings);
    ...
    validateExtensionPolicyIdentity(identityKey) {
