...
function validateRpcCommand(value) {
...
function rpcCommandErrorMetadata(value) {
    if (!isJsonObject(value))
        return { id: undefined, type: "schema" };
    return {
...
function validateImageArray(value, path) {
    if (value === undefined)
        return;
...
function expectRequiredBoolean(object, parentPath, field) {
    const value = requiredValue(object, parentPath, field);
...
export async function runRpcMode(runtimeHost) {
    ...
    const handleInputLine = async (line) => {
        ...
        try {
            ...
            output(error(metadata.id, metadata.type, schemaError instanceof Error ? schemaError.message : String(schemaError)));
            return;
        }
        try {
