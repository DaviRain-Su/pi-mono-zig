...
sequenceDiagram
    ...
    participant Hook as tool_call hook<br/>(可选扩展拦截)
    ...
    Loop->>Hook: 触发 tool_call (Phase 1 钩子)
    Hook-->>Loop: 通过 / 拦截 / 改参数
    Loop->>Cap: 检查 principal 有 file.write?
    Cap-->>Loop: yes
    ...
    Loop->>Hook: 触发 tool_result (Phase 1 钩子)
    Hook-->>Loop: 通过 / 改写结果
...
- 第 7 章的 `tool_call` / `tool_result` 拦截
...
flowchart TB
    ...
    L1 --> L1d["principal 没有 shell.run grant？<br/>立刻 deny，根本到不了 bash 工具"]

    L2[第二层：扩展拦截<br/>(tool_call hook)]:::l2
