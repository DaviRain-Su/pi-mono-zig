...
const long_function_allowlist = [_]AllowlistedLongFunction{
    ...
    .{ .path = "src/coding_agent/packages/package_manager.zig", .function_name = "parsePackageCommand", .reason = "pre-existing package command parser debt tracked by exact function allowlist so new long functions still fail tidy" },
    .{ .path = "src/coding_agent/sessions/session_advanced.zig", .function_name = "renderSessionHtml", .reason = "pre-existing session HTML renderer debt tracked by exact function allowlist so new long functions still fail tidy" },
