...
pub fn queryTerminalSend(vx: *Vaxis, tty: *std.Io.Writer) !void {
    ...
    try tty.writeAll(queryTerminalSequence(shouldQueryKittyKeyboard(vx)));

    try tty.flush();
}

fn queryTerminalSequence(include_kitty_keyboard_query: bool) []const u8 {
    return if (include_kitty_keyboard_query)
...
fn shouldQueryKittyKeyboard(vx: *const Vaxis) bool {
    return !isGhosttyEnvMap(vx.env_map);
}

fn isGhosttyEnvMap(env_map: *const std.process.Environ.Map) bool {
    return isGhosttyEnv(
        env_map.get("TERM_PROGRAM"),
        env_map.get("GHOSTTY_RESOURCES_DIR"),
        env_map.get("TERM"),
    );
}

fn isGhosttyEnv(term_program: ?[]const u8, ghostty_resources_dir: ?[]const u8, term: ?[]const u8) bool {
    if (term_program) |value| {
        if (std.ascii.eqlIgnoreCase(value, "Ghostty")) return true;
    }
    if (ghostty_resources_dir != null) return true;
    if (term) |value| {
        if (std.ascii.indexOfIgnoreCase(value, "ghostty") != null) return true;
    }
    return false;
}
...
pub fn enableDetectedFeatures(self: *Vaxis, tty: *std.Io.Writer) !void {
    switch (builtin.os.tag) {
        .windows => {
            ...
            if (isGhosttyEnvMap(self.env_map))
                self.caps.kitty_keyboard = false;
...
test "queryTerminalSend includes Kitty keyboard query outside Ghostty" {
    ...
    var deinit_writer: std.Io.Writer.Allocating = .init(allocator);
    defer deinit_writer.deinit();
    defer vx.deinit(allocator, &deinit_writer.writer);

    var query_writer: std.Io.Writer.Allocating = .init(allocator);
    defer query_writer.deinit();
    try vx.queryTerminalSend(&query_writer.writer);

    const output = query_writer.written();
    try std.testing.expect(shouldQueryKittyKeyboard(&vx));
    try std.testing.expect(std.mem.indexOf(u8, output, ctlseqs.csi_u_query) != null);
    try std.testing.expect(std.mem.indexOf(u8, output, ctlseqs.kitty_graphics_query) != null);
}

test "enableDetectedFeatures enables detected Kitty keyboard outside Ghostty" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var env_map = std.process.Environ.Map.init(allocator);
    defer env_map.deinit();
    ...
    var enable_writer: std.Io.Writer.Allocating = .init(allocator);
    defer enable_writer.deinit();
    try vx.enableDetectedFeatures(&enable_writer.writer);

    try std.testing.expect(vx.caps.kitty_keyboard);
    try std.testing.expect(vx.state.kitty_keyboard);
    try std.testing.expect(std.mem.indexOf(u8, enable_writer.written(), "\x1b[>0u") != null);
}

test "Ghostty environment suppresses Kitty keyboard query and enable" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var env_map = std.process.Environ.Map.init(allocator);
    defer env_map.deinit();
    ...
    defer query_writer.deinit();
    try vx.queryTerminalSend(&query_writer.writer);
    const query_output = query_writer.written();
    try std.testing.expect(!shouldQueryKittyKeyboard(&vx));
    try std.testing.expect(std.mem.indexOf(u8, query_output, ctlseqs.csi_u_query) == null);
    try std.testing.expect(std.mem.indexOf(u8, query_output, ctlseqs.kitty_graphics_query) != null);
...
test "Ghostty detection covers env map signals" {
    try std.testing.expect(!isGhosttyEnv(null, null, null));
    try std.testing.expect(isGhosttyEnv("Ghostty", null, null));
    try std.testing.expect(isGhosttyEnv("ghostty", null, null));
    try std.testing.expect(isGhosttyEnv(null, "/usr/share/ghostty", null));
    try std.testing.expect(isGhosttyEnv(null, null, "xterm-GHOSTTY"));
}
