        ...
        fn _ttyRun(
            ...
            switch (builtin.os.tag) {
                .windows => {
                    ...
                    var parser: Parser = .{};
                    const raw_debug_path = rawInputDebugPath(self.vaxis);
                    ...
                    read_loop: while (!self.should_quit) {
                        ...
                        if (raw_debug_path) |path| {
                            writeRawInputDebugRead(self.io, path, buf[read_start..total]) catch {};
                        }
                        ...
                        while (seq_start < total) {
                            ...
                            if (raw_debug_path) |path| {
                                const event_tag = if (result.event) |event|
                                    @tagName(std.meta.activeTag(event))
                                else
                                    "none";
                                writeRawInputDebugParse(self.io, path, result.n, event_tag) catch {};
                            }
                            if (result.n == 0) {
...
fn rawInputDebugPath(vx: *const Vaxis) ?[]const u8 {
    const path = vx.env_map.get("PI_RAW_INPUT_DEBUG_LOG") orelse return null;
    if (std.mem.trim(u8, path, " \t\r\n").len == 0) return null;
    return path;
}

fn writeRawInputDebugRead(io: std.Io, path: []const u8, bytes: []const u8) !void {
    var line: [8192]u8 = undefined;
    var index: usize = 0;

    try appendFmt(&line, &index, "read bytes={d} hex=", .{bytes.len});
    for (bytes, 0..) |byte, byte_index| {
        if (byte_index > 0) try appendByte(&line, &index, ' ');
        try appendFmt(&line, &index, "{x:0>2}", .{byte});
    }

    try appendSlice(&line, &index, " repr=\"");
    for (bytes) |byte| {
        switch (byte) {
            '\n' => try appendSlice(&line, &index, "\\n"),
            '\r' => try appendSlice(&line, &index, "\\r"),
            '\t' => try appendSlice(&line, &index, "\\t"),
            '"' => try appendSlice(&line, &index, "\\\""),
            '\\' => try appendSlice(&line, &index, "\\\\"),
            else => {
                if (byte >= 0x20) {
                    try appendByte(&line, &index, byte);
                } else {
                    try appendFmt(&line, &index, "\\x{x:0>2}", .{byte});
                }
            },
        }
    }
    ...
    try appendRawInputDebugLine(io, path, line[0..index]);
}

fn writeRawInputDebugParse(io: std.Io, path: []const u8, consumed: usize, event_tag: []const u8) !void {
    var line: [128]u8 = undefined;
    const text = try std.fmt.bufPrint(&line, "parse consumed={d} event={s}\n", .{ consumed, event_tag });
    try appendRawInputDebugLine(io, path, text);
}

fn writeRawInputDebugParseError(io: std.Io, path: []const u8, byte: u8, err: anyerror) !void {
    var line: [128]u8 = undefined;
    const text = try std.fmt.bufPrint(&line, "parse error={s} byte=0x{x:0>2}\n", .{ @errorName(err), byte });
    try appendRawInputDebugLine(io, path, text);
}

fn appendRawInputDebugLine(io: std.Io, path: []const u8, line: []const u8) !void {
...
fn appendSlice(buffer: []u8, index: *usize, text: []const u8) !void {
...
fn appendFmt(buffer: []u8, index: *usize, comptime fmt: []const u8, args: anytype) !void {
    const text = try std.fmt.bufPrint(buffer[index.*..], fmt, args);
    index.* += text.len;
}

fn carryPendingBytes(buf: []u8, seq_start: usize, total: usize) usize {
    const pending_len = total - seq_start;
    std.mem.copyForwards(u8, buf[0..pending_len], buf[seq_start..total]);
    return pending_len;
}

test "carryPendingBytes preserves all unconsumed bytes" {
    var buf = [_]u8{ 'a', 0xE4, 0xBD, 'z', 0, 0 };

    const read_start = carryPendingBytes(buf[0..], 1, 4);

    try std.testing.expectEqual(3, read_start);
    try std.testing.expectEqualSlices(u8, &[_]u8{ 0xE4, 0xBD, 'z' }, buf[0..read_start]);
}
