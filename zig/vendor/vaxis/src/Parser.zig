...
inline fn parseGround(input: []const u8) !Result {
    ...
    const key: Key = switch (b) {
        ...
        0x7F => .{ .codepoint = Key.backspace },
            ...
            if (isIncompleteUtf8Prefix(input)) {
                return .{
                    .event = null,
                    .n = 0,
                };
...
inline fn isIncompleteUtf8Prefix(input: []const u8) bool {
    const expected_len = std.unicode.utf8ByteSequenceLength(input[0]) catch return false;
    if (expected_len == 1 or input.len >= expected_len) return false;

    switch (input[0]) {
        0xC2...0xDF => {
            return input.len == 1 or isContinuationByte(input[1]);
        },
        0xE0 => {
            if (input.len == 1) return true;
            return input[1] >= 0xA0 and input[1] <= 0xBF;
        },
        0xE1...0xEC, 0xEE...0xEF => {
            if (input.len == 1) return true;
            return isContinuationByte(input[1]);
        },
        0xED => {
            if (input.len == 1) return true;
            return input[1] >= 0x80 and input[1] <= 0x9F;
        },
        0xF0 => {
            if (input.len == 1) return true;
            if (input[1] < 0x90 or input[1] > 0xBF) return false;
            return input.len == 2 or isContinuationByte(input[2]);
        },
        0xF1...0xF3 => {
            if (input.len == 1) return true;
            if (!isContinuationByte(input[1])) return false;
            return input.len == 2 or isContinuationByte(input[2]);
        },
        0xF4 => {
            if (input.len == 1) return true;
            if (input[1] < 0x80 or input[1] > 0x8F) return false;
            return input.len == 2 or isContinuationByte(input[2]);
        },
        else => return false,
    }
...
test "parse: incomplete Chinese codepoint waits for more bytes" {
    const alloc = testing.allocator_instance.allocator();
    var parser: Parser = .{};

    {
        const result = try parser.parse("\xE4", alloc);
        try testing.expectEqual(0, result.n);
        try testing.expectEqual(@as(?Event, null), result.event);
    }

    {
        const result = try parser.parse("\xE4\xBD", alloc);
        try testing.expectEqual(0, result.n);
        try testing.expectEqual(@as(?Event, null), result.event);
    }
}

test "parse: full Chinese codepoint produces keypress" {
    const alloc = testing.allocator_instance.allocator();
    const input = "你";
    var parser: Parser = .{};
    const result = try parser.parse(input, alloc);
