...
const PromptEventCapture = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, host: *BridgeHost, session_id: []const u8, turn_id: []const u8) PromptEventCapture {
        return .{
            .allocator = allocator,
            .host = host,
            .session_id = session_id,
    ...
    pub fn deinit(self: *PromptEventCapture) void {
        if (self.terminal_error_message) |message| self.allocator.free(message);
        self.* = undefined;
    }

    pub fn appendEvent(self: *PromptEventCapture, event: agent.AgentEvent) !void {
    ...
    pub fn appendSyntheticTerminal(self: *PromptEventCapture, outcome: []const u8, message: []const u8) !void {
        if (self.terminal_seen) return;
        self.terminal_seen = true;
...
pub const testing = struct {
    pub const PromptEventCaptureType = PromptEventCapture;

    pub fn dispatchCounterTotal(counters: DispatchCounters) usize {
        return counters.total();
    }

    pub fn enqueueEventFrame(
        host: *BridgeHost,
        frame: []u8,
        sequence: usize,
        terminal: bool,
        ...
        return host.enqueueEventFrame(frame, sequence, terminal, terminal_outcome, terminal_error_message);
    }

    pub fn eventFrameCount(host: *const BridgeHost) usize {
        return host.event_frames.items.len;
    }

    pub fn eventFrameBytes(host: *const BridgeHost, index: usize) []const u8 {
        return host.event_frames.items[index].bytes;
    }
};

test {
    _ = @import("webview_bridge/tests.zig");
}
