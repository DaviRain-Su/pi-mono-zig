...
pub fn thinkingLevelSupported(model: types.Model, level: types.ModelThinkingLevel) bool {
    ...
    const mapping = thinkingLevelMapping(model, level);
    if (mapping) |value| {
...
pub fn clampThinkingLevel(model: types.Model, level: types.ModelThinkingLevel) types.ModelThinkingLevel {
    if (thinkingLevelSupported(model, level)) return level;
    const levels = [_]types.ModelThinkingLevel{ .off, .minimal, .low, .medium, .high, .xhigh };
    const requested_index = for (levels, 0..) |candidate, index| {
        if (candidate == level) break index;
    } else 0;

    var i = requested_index;
    while (i < levels.len) : (i += 1) {
        if (thinkingLevelSupported(model, levels[i])) return levels[i];
    }
...
pub fn supportsXhigh(model: types.Model) bool {
    if (thinkingLevelMapping(model, .xhigh)) |mapping| {
...
const BUILT_IN_MODELS = [_]ModelDefinition{
    ...
    .{ .provider = "moonshotai", .id = "kimi-k2.6", .name = "Kimi K2.6", .reasoning = true, .input_types = TEXT_AND_IMAGE_INPUTS[0..], .context_window = 262144, .max_tokens = 262144 },
    .{ .provider = "moonshotai-cn", .id = "kimi-k2.6", .name = "Kimi K2.6", .reasoning = true, .input_types = TEXT_AND_IMAGE_INPUTS[0..], .context_window = 262144, .max_tokens = 262144 },
    ...
    .{ .provider = "cloudflare-workers-ai", .id = "@cf/moonshotai/kimi-k2.6", .name = "Kimi K2.6", .reasoning = true, .input_types = TEXT_AND_IMAGE_INPUTS[0..], .context_window = 256000, .max_tokens = 256000 },
    .{ .provider = "cloudflare-ai-gateway", .id = "workers-ai/@cf/moonshotai/kimi-k2.6", .name = "Kimi K2.6", .reasoning = true, .input_types = TEXT_AND_IMAGE_INPUTS[0..], .context_window = 256000, .max_tokens = 256000 },
